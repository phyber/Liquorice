...
local function AUCTION_HOUSE_SHOW(frame, event, ...)
    ...
    -- Buyout Sort
    local BrowseBuyoutSort = CreateFrame(
        "Button",
        "BrowseBuyoutSort",
        AuctionFrameBrowse,
    ...
    -- ItemName sort
    local BrowseNameSort = CreateFrame(
        "Button",
        "BrowseNameSort",
        AuctionFrameBrowse,
        "AuctionSortButtonTemplate"
    ...
    BrowseQualitySort:SetWidth(
        BrowseQualitySort:GetWidth() - BrowseNameSort:GetWidth()
    )
    ...
    -- Finally, remove the SetScript so we don't do this every time we open
    -- the AH window.
